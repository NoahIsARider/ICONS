import { artists, starters, venues, events, albumWordsA, albumWordsB, phaseNames } from './data.mjs';
...
export function newGame(config, rng = Math.random) {
  ...
  if (entries.length < 2 || entries.length > 4 || !entries.some(p => !p.ai)) throw new Error('Choose 2–4 labels with at least one human player.');
...
function nextPhase(state) {
  state.phase++;
  state.current = 0;
  if (state.phase === 2) pushLog(state, 'All labels have completed Training & Care. Reveal the event.', 'highlight');
  if (state.phase === 5) pushLog(state, 'The show circuit closes. Resolve the Grammy Awards.', 'highlight');
}
...
function checkCollaboration(state, player, newArtist, rng) {
  ...
  if (test >= 3) {
    ...
    pushLog(state, `${player.name} signed a collaboration! ${newArtist.name} and ${partner.name} released a single.`, 'success');
...
function resolveAuction(state, rng) {
  ...
  } else pushLog(state, `No label bid on ${lot.name}. The artist remains independent.`);
...
function train(state, action) {
  ...
  if (action.type === 'pass') { pushLog(state, `${player.name} skipped training.`); nextPlayer(state); return; }
  const artist = owned(player, action.artistId);
  if (!artist) throw new Error('Choose one of your living artists.');
  ...
  if (kind === 'care' && artist.currentStamina === artist.maxStamina && !Object.values(artist.status).some(Boolean)) throw new Error('This artist needs no care.');
  player.money -= cost;
  if (kind === 'care') {
  ...
  pushLog(state, `${player.name} gave ${artist.name} ${kind === 'care' ? 'care' : `${kind} training`} for $${cost}.`, 'success');
  nextPlayer(state);
}
...
function eventPhase(state, rng) {
  ...
  if (event.id === 'backstage' || event.id === 'smoky' || event.id === 'flu') {
    ...
    if (target) {
      ...
      pushLog(state, `${target.name} gained ${event.id === 'backstage' ? 'Banned Substances and Stress' : event.id === 'smoky' ? 'Tobacco' : 'Illness'}.`, 'danger');
    }
  }
  nextPhase(state);
}
...
function create(state, action, rng) {
  const player = currentPlayer(state);
  if (action.type === 'pass') { pushLog(state, `${player.name} skipped album creation.`); nextPlayer(state); return; }
  const artist = owned(player, action.artistId);
  if (!artist || !canCreate(state, player, artist)) throw new Error('That artist cannot create an album now.');
  ...
  pushLog(state, `${artist.name} released “${work.title}” (quality ${quality}, +$${royalty}/round, +${acclaim} acclaim).`, 'success');
  nextPlayer(state);
}
...
function performance(state, action, rng) {
  ...
  if (!venue || !canPlay(state,player,artist,venue)) throw new Error('Artist or venue is unavailable.');
  ...
  if (artist.currentStamina === 0) artist.status.exhaustion = 1;
  pushLog(state, `${artist.name} played ${venue.name} for ${player.name}, earning $${earned}.`, 'success');
  nextPlayer(state);
}
...
function awards(state) {
  ...
  if (works.length) {
    ...
    pushLog(state, `GRAMMY — ${winner.title} by ${player.name} wins! +3 acclaim and a cash prize.`, 'highlight');
  } else pushLog(state, 'GRAMMY — No new release was eligible this round.');
...
function critics(state) {
  ...
  else pushLog(state, 'CRITICS — Every label is tied. No penalty.');
  nextPhase(state);
}
...
function karma(state, rng) {
  for (const player of state.players) for (const artist of living(player)) {
    ...
    if (test + 2 <= risk && (artist.status.illness || artist.status.drugs)) {
      artist.dead = true;
      pushLog(state, `KARMA — ${artist.name} died after a serious health crisis.`, 'danger');
      ...
      pushLog(state, `KARMA — ${artist.name} suffered a permanent ability loss.`, 'danger');
    }
  }
  nextPhase(state);
}
...
function reset(state) {
  for (const player of state.players) {
    const royalties = player.works.reduce((sum,work) => sum + work.royalty + (state.event?.id === 'streaming' ? 2 : 0),0);
    player.money += royalties;
    if (royalties) pushLog(state, `${player.name} received $${royalties} in royalties.`, 'success');
...
export function act(state, action, rng = Math.random) {
  ...
  switch (state.phase) {
    ...
    case 3: if (!['create','pass'].includes(action.type)) throw new Error('Create an album or pass.'); create(state,action,rng); break;
