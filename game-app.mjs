...
const KEY = 'record-label-rivals-save-v1';
const escapeHtml = value => String(value ?? '').replace(/[&<>"']/g, char => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));
const icon = (name, className = '') => `<img class="${className}" src="icons/${name}.png" alt="" draggable="false">`;
const portrait = artist => `portraits/${artist.id}.jpg`;
...
try {
  const desktopSave = window.recordLabelDesktop ? await window.recordLabelDesktop.loadSave() : null;
  const saved = JSON.parse(desktopSave || localStorage.getItem(KEY));
...
function save() {
  if (state) {
    ...
    window.recordLabelDesktop?.saveGame(data).catch(error=>console.error('Save failed:',error));
  } else {
    localStorage.removeItem(KEY);
    window.recordLabelDesktop?.clearSave().catch(error=>console.error('Save removal failed:',error));
  }
}
...
function artistCard(artist,{size='hand',selected=false,draggable=false,showOwner=false}={}) {
  ...
  return `<button class="playing-card ${size} ${selected?'selected':''} ${artist.dead?'dead':''}" data-action="select-artist" data-id="${escapeHtml(artist.id)}" ${draggable?'draggable="true"':''} type="button" aria-label="Select ${escapeHtml(artist.name)}">
    ...
    <span class="card-trait">${icon(traitIcons[artist.tag]||'01-recruitment')}<span>${escapeHtml(artist.tag)}</span></span>
    <span class="card-info"><b>${escapeHtml(artist.name)}</b><span class="card-stats">${miniStat('13-vocal',artist.vocal,'VOCAL')}${miniStat('14-creativity',artist.creativity,'CREATE')}${miniStat('15-stamina',artist.currentStamina ?? artist.stamina,'ENERGY')}</span></span>
    ${status.length?`<span class="card-status">${status.map(escapeHtml).join(' · ')}</span>`:''}
    ${artist.dead?'<span class="card-dead">IN MEMORIAM</span>':''}
...
function setupScreen() {
  return `<main class="title-screen"><div class="title-vignette"></div><div class="title-content">
    <div class="title-kicker">A RECORD LABEL STRATEGY GAME</div>
    <h1>RECORD LABEL<br><em>RIVALS</em></h1>
    <div class="title-rule"></div>
    <p>Discover the voices. Press the records. Own the stage.<br>The richest label after ten rounds takes the crown.</p>
    ...
    </form><div class="title-foot">LOCAL MULTIPLAYER · SOLO VS COMPUTER · AUTOSAVE</div>
...
function recruitmentScene() {
  ...
  return `<div class="scene scene-auction">
    ${instruction('THE NEXT BIG THING',`Lot ${state.auctionIndex+1} of 2 · ${submitted}/${state.players.length} sealed bids placed`)}
    ...
    <div class="auction-console"><span class="console-kicker">SEALED BID</span><p>Recruit ${escapeHtml(lot.name)}. Highest bid signs the artist. Losing bids cost nothing.</p><div class="bid-dial"><button data-action="bid-down" aria-label="Decrease bid">−</button><strong>$${bidAmount}</strong><button data-action="bid-up" aria-label="Increase bid">+</button></div><input id="bid-range" type="range" min="0" max="${player.money}" value="${bidAmount}" aria-label="Bid amount"><div class="bid-limits"><span>PASS · $0</span><span>YOUR CASH · $${player.money}</span></div><div class="bid-limits"><span id="bid-left">CASH LEFT · $${player.money - bidAmount}</span><span>TRAINING $3 · ALBUM $${roundCost()}</span></div><button class="gold-button bid-submit" data-action="bid">SEAL THE BID <span>➜</span></button></div>
    <div class="auction-decoration">${icon('01-recruitment')}<span>ARTIST AUCTION</span></div>
  </div>`;
}
...
function trainingScene() {
  ...
  const actions = [
    ['creativity','14-creativity','Creative Coaching','$3','+1 Creativity'],
    ['vocal','13-vocal','Vocal Coaching','$3','+1 Vocal'],
  ...
  const reason = (kind, cost) => {
    if (!artist) return 'Select an artist first';
  ...
  return `<div class="scene scene-training">${instruction('SHAPE A STAR','Select an artist from your hand. Click a training tile or drag the card onto it.')}
    <div class="training-portrait">${artist?artistCard(artist,{size:'feature',selected:true}):'<div class="empty-card">NO ARTISTS AVAILABLE</div>'}</div>
...
function eventScene() {
  return `<div class="scene scene-event">${instruction('FATE HAS A NEW TRACK','A single event changes the rhythm of this round. Flip the card to reveal it.')}
...
function creationScene() {
  ...
  return `<div class="scene scene-creation">${instruction('MAKE THE NEXT CLASSIC',`Select a ready artist. A new album costs $${roundCost()} and generates royalties every round.`)}
    <div class="creation-artist">${artist?artistCard(artist,{size:'feature',selected:true}):'<div class="empty-card">NO ARTISTS AVAILABLE</div>'}</div>
    <button class="studio-press drop-zone ${can?'ready':'unavailable'}" data-action="create" data-drop="create" ${!anyone?'disabled':''} aria-label="Release an album with selected artist"><span class="spinning-vinyl"><span></span></span><b>PRESS A RECORD</b><small>$${roundCost()} STUDIO COST · +ROYALTIES · +ACCLAIM</small></button>
...
function venueTile(venue,artist,player) {
  ...
  return `<button class="venue-tile drop-zone ${eligible?'eligible':''} ${occupied?'occupied':''}" data-action="venue" data-venue="${venue.id}" data-drop="venue" ${!anyone?'disabled':''}>
    <span class="venue-light"></span>${icon(venue.icon)}<span class="venue-name">${escapeHtml(venue.name)}</span><span class="venue-pay">$${venue.pay} <small>BASE</small></span><span class="venue-needs">VOCAL ${venue.vocal}+ · ${venue.stamina} ENERGY</span><span class="venue-marker">${occupied?'TAKEN':venue.tour?'SOLO TOUR':eligible?'PLACE ARTIST':anyone?'SELECT ARTIST':'LOCKED'}</span>
  </button>`;
}

function showScene() {
  const artist = selectedArtist(), player = currentPlayer(state);
  return `<div class="scene scene-shows">${instruction('CLAIM THE STAGE',`Booking ${state.showCycle+1} of 2. Select an artist, then claim an open venue. Each artist may play once per round; shared stages fill when claimed.`)}
    <div class="venues-board">${venues.map(venue=>venueTile(venue,artist,player)).join('')}</div><button class="scene-pass" data-action="pass">PASS LIVE SHOW ➜</button>
  </div>`;
}
...
function awardsScene() {
  ...
  return `<div class="scene scene-ceremony">${instruction('AND THE WINNER IS…','The strongest new record claims the Grammy. Empress breaks quality ties. Acclaim pays every label.')}
    <div class="ceremony-trophy">${icon('06-awards')}<span>THE GRAMMY</span></div><div class="nominees">${candidates.length?candidates.slice(0,3).map(work=>`<div class="nominee"><span class="nominee-disc">◉</span><b>${escapeHtml(work.title)}</b><small>${escapeHtml(work.player.name)} · Q${work.quality}</small></div>`).join(''):'<div class="nominee empty">NO NEW RELEASES</div>'}</div>${goldAction('OPEN THE ENVELOPE')}
  </div>`;
}

function criticsScene() {
  const max = Math.max(1,...state.players.map(player=>player.acclaim));
  return `<div class="scene scene-critics">${instruction('THE CRITICS HAVE SPOKEN','The label at the bottom of the acclaim chart pays a penalty.')}
...
function karmaScene() {
  ...
  return `<div class="scene scene-karma">${instruction('THE COST OF FAME','Exhaustion, illness, stress and banned substances can leave permanent marks. Roll for every affected artist.')}
    <div class="risk-cards">${risks.length?risks.slice(0,5).map(({artist,player})=>`<div class="risk-card"><img src="${portrait(artist)}" alt=""><span><b>${escapeHtml(artist.name)}</b><small>${escapeHtml(player.name)}</small><em>${Object.entries(artist.status).filter(([,value])=>value).map(([key])=>escapeHtml(key)).join(' · ')}</em></span></div>`).join(''):`<div class="risk-clear">${icon('08-karma')}<b>NO ARTISTS AT RISK</b></div>`}</div>${goldAction('ROLL THE CONSEQUENCES')}
  </div>`;
}

function resetScene() {
  return `<div class="scene scene-reset">${instruction('THE RECORDS KEEP SPINNING','Collect royalties from every released record. Artists recover energy before the next round.')}
    <div class="royalty-stack">${state.players.map(player=>{const amount=player.works.reduce((sum,work)=>sum+work.royalty+(state.event?.id==='streaming'?2:0),0);return `<div class="royalty-line">${icon('11-royalties')}<b>${escapeHtml(player.name)}</b><strong>+$${amount}</strong><small>${player.works.length} releases</small></div>`;}).join('')}</div>${goldAction(state.round===10?'REVEAL THE WINNER':'START THE NEXT ROUND')}
  </div>`;
}

function gameOverScene() {
  const sorted = [...state.players].sort((a,b)=>b.money-a.money||b.acclaim-a.acclaim);
  return `<div class="scene scene-finale"><div class="finale-rays"></div>${icon('06-awards','finale-trophy')}<span class="finale-kicker">THE FINAL CHART</span><h1>${escapeHtml(state.winnerIds.map(id=>state.players[id].name).join(' & '))}</h1><p>TAKES THE CROWN</p><div class="finale-scores">${sorted.map((player,index)=>`<div><span>#${index+1}</span><b>${escapeHtml(player.name)}</b><strong>$${player.money}</strong><small>★ ${player.acclaim}</small></div>`).join('')}</div><button class="gold-button" data-action="new-game">PLAY AGAIN ➜</button></div>`;
}
...
function hand() {
  ...
  return `<div class="hand-zone"><div class="hand-heading"><span>YOUR ARTISTS <b>${escapeHtml(player.name)}</b></span><button data-action="rosters">VIEW ALL LABELS ↗</button></div><div class="hand-cards">${cards.length?cards.map((artist,index)=>`<div class="hand-holder" style="--index:${index};--total:${cards.length}">${artistCard(artist,{selected:artist.id===selectedId,draggable:true})}</div>`).join(''):'<div class="hand-empty">No living artists. Pass this action and recruit again next round.</div>'}</div><div class="hand-hint">${[1,3,4].includes(state.phase)?'CLICK A CARD TO SELECT · DRAG IT TO AN ACTION SPACE':'YOUR ROSTER'}</div></div>`;
}
...
function modalOverlay() {
  if (!modal) return '';
  if (modal === 'menu') return `<div class="veil modal-veil"><div class="game-modal"><span class="modal-kicker">PAUSE MENU</span><h2>Record Label Rivals</h2><button data-action="rules">HOW TO PLAY</button><button data-action="rosters">ALL LABELS</button><button data-action="sound">SOUND: ${soundOn?'ON':'OFF'}</button><button data-action="confirm-new">NEW GAME</button><button data-action="close-modal">RETURN TO TABLE</button></div></div>`;
  ...
  if (modal === 'rules') return `<div class="veil modal-veil"><div class="game-modal rules-modal"><h2>How to Play</h2><p>After ten rounds, the label with the most cash wins. Each round: bid for two artists, train or care for one, reveal an event, release one album, place one artist at a show, resolve the Grammy, critics, karma, and royalties.</p><p>Albums earn acclaim now and royalties every round. Shows pay cash immediately. Solo Tour unlocks once that artist has a release. Shared show venues fill when claimed.</p><p>Artpop improves creation. Vocal Flip improves shows. Jazz ignores Tobacco's Vocal penalty. Empress wins award ties. Related artists can collaborate and pass on a weaker talent after a mentor dies.</p><button data-action="menu">BACK TO MENU</button><button data-action="close-modal">RETURN TO TABLE</button></div></div>`;
  if (modal === 'rosters') return `<div class="veil modal-veil"><div class="roster-modal"><div class="roster-top"><h2>Signed Artists</h2><button data-action="close-modal">✕</button></div><div class="all-rosters">${state.players.map(player=>`<section><h3 style="--chip:${player.color}">${escapeHtml(player.name)} <span>$${player.money} · ★ ${player.acclaim} · ${player.works.length} releases</span></h3><div>${player.artists.map(artist=>artistCard(artist,{size:'roster'})).join('')}</div></section>`).join('')}</div></div></div>`;
  return '';
}

function render() {
  if (!state) { root.innerHTML = setupScreen(); return; }
  root.innerHTML = `<div class="game-table">${seatBar()}${phaseStrip()}<main class="table-surface">${scene()}</main>${hand()}<div class="table-corner left">SIDE A · ORIGINAL PRESSING</div><div class="table-corner right">10 ROUNDS · MOST CASH WINS</div>${notice?`<div class="game-notice ${notice.type}">${icon(notice.type==='danger'?'08-karma':notice.type==='event'?'03-event':'11-royalties')}<span>${escapeHtml(notice.text)}</span></div>`:''}${gateOverlay()}${modalOverlay()}</div>`;
}
