...
```
I made a tabletop card game about running a record label: 10 rounds, 24 artists, sealed bids where only the winner pays.
...
Attach: `screenshots/01-title-screen.jpg`

Alt text: Title screen: the gold RECORD LABEL RIVALS logo, a Choose 2-4 labels form, and four artist cards fanned out on a navy Art Deco table.

### EN post 2 — what the simulations say

**180 / 280 characters**

```
What 4,400 simulated games say:
...
Attach: `screenshots/06-live-show-board.jpg`, `screenshots/02-artist-auction.jpg`

Alt text: Left: the live show board with five venue tiles from Studio Session $4 to Grand Arena $12. Right: the sealed-bid auction, with a bid dial reading $8 and the note that losing bids cost nothing.

### EN post 3 — the flaw it found

**254 / 280 characters**

```
The one balance flaw it found: with four equally good players, the first seat wins 50.5% of games instead of 25%.
...
### EN post 4 — the counter-intuitive design bit

**249 / 280 characters**

```
A design fact I only learned by measuring: the four trait tags are worth about 6% of your final cash.
...
### EN post 5 — the download

**168 / 280 characters**

```
Download: Windows x64, 173 MB, unzip and run.
...
Attach: `screenshots/08-winner-finale.jpg`

Alt text: The final chart: the winner's name above the ranked scores, Your Label $268 and ★31 acclaim against Rival Records $214 and ★24.

### EN post 6 — the repo and the ask

**240 / 280 characters**

```
Source, all 24 artist card portraits, 27 print-ready icon tokens, and a strategy guide measured from those 4,400 games:
...
Attach: `screenshots/09-icon-set.jpg`, `screenshots/10-artist-portraits.jpg`

Alt text: Two artwork sheets: the 27 gold-on-navy game icon medallions, and twenty-four illustrated artist portraits with their names and genres.
...
```
做了个桌面卡牌游戏《Record Label Rivals》：经营一家唱片公司，10 个回合里抢艺人、发专辑、跑演出、冲格莱美。竞价是密封的，只有赢家付钱。
...
Alt text: 标题界面：金色的 RECORD LABEL RIVALS 标志、2-4 家公司的选择表单，以及深蓝 Art Deco 桌面上摊开的四张艺人卡。
...
Attach: `screenshots/06-live-show-board.jpg`

Alt text: 演出回合的界面：五张场地牌，从 Studio Session $4 到 Grand Arena $12，玩家需要在其中安排自己的艺人。
...
```
唯一测出的平衡缺陷：四个同水平玩家时，1 号座位胜率 50.5%，而公平值应该是 25%。
...
```
下载（Windows x64，173MB，解压即玩；未签名会被 SmartScreen 拦，点 More info → Run anyway）：
...
Attach: `screenshots/09-icon-set.jpg`

Alt text: 两张美术图：27 个金线深蓝底的图标圆章，以及 24 位艺人的立绘与流派。
